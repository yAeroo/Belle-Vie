...
<a href="{{ route('profile', $user) }}"></a>
    ...
    <div class="py-8 relative object-center max-w-2xl mx-auto">
        ...
        <button class="absolute top-1/2 left-0 transform -translate-y-1/2 px-4 text-white bg-[#E95F4A] rounded-md hover:bg-gray-800" onclick="document.querySelector('.overflow-x-auto').scrollBy({left: -600, behavior: 'smooth'})">
          &lt;
        </button>
        <button class="absolute top-1/2 right-0 transform -translate-y-1/2 px-4 text-white bg-[#E95F4A] rounded-md hover:bg-gray-800" onclick="document.querySelector('.overflow-x-auto').scrollBy({left: 600, behavior: 'smooth'})">
          &gt;
        </button>
    </div>
